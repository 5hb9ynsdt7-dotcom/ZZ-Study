...
import { Card, Button, Typography, Row, Col, Tabs, Tag } from 'antd';
...
const ChinesePage = () => {
  ...
  const { speak } = useSpeechSynthesis('zh-CN');

  // 朗读拼音
  const speakPinyin = useCallback((text) => {
    const isChineseChar = /[\u4e00-\u9fa5]/.test(text);
    const textToSpeak = isChineseChar ? text : (PINYIN_PRONUNCIATION[text] || text);
    
    speak(textToSpeak, {
      rate: SPEECH_CONFIG.CHINESE.defaultRate,
      preferredVoices: SPEECH_CONFIG.CHINESE.preferredVoices,
    });
  }, [speak]);
